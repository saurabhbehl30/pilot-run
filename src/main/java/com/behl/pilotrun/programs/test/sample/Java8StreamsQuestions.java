...
public class Java8StreamsQuestions {
    ...
    public static void main(String[] args) {
        ...
        List<Person> persons = Arrays.asList(
                new Person("Alice", 25),
                new Person("Bob", 35),
                new Person("harlie", 28),
                new Person("ADavid", 40)
        );
        ...
        Map<Boolean, List<Integer>> partitionedNumbers = numbers.stream()
                .collect(Collectors.partitioningBy(n -> n % 2 == 0));  // partion for even odd
      ...
      System.out.println(wordLongest);
        ...
        double averageLength = words.stream()
                .filter(s -> "aeiou".contains(String.valueOf(s.charAt(0)).toLowerCase()))
